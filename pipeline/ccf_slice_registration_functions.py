...
def visualize_landmarks( image_2d, ax, landmarks = [], plot_style = []) :
    ...
    for i, p in enumerate(landmarks):
        ax.plot(p[0],p[1],plot_style[i],markersize=10, fillstyle='none')
...
def get_20x_img(sub_image, image_path, downsample=0):
    sql ="""
...
def register_morph(sp_name, sp_id, lims_soma, morph, swc_path, swc_name, cell_soma_info, overview_to_virtual_slice_transform, virtual_slice_to_ccf_transform):
    ...
    #save ccf registered cell to swc 
    registered_swc_path = os.path.join(swc_path, swc_name+'_ccf.swc')
    dict_to_swc(morph, registered_swc_path)

    print('\t\tRegistration complete!')

    return registered_swc_path
    

# add today's date to the end of the file name 
# ex: fileName.pdf --> fileName_20230622.pdf 
# date format is yyyymmdd
def addDateToFileName(file_name):
    split_file_name = file_name.rsplit('.', 1)
    ct = datetime.datetime.now()
    new_file_name = split_file_name[0] + '_{}{:02d}{:02d}.'.format(ct.year, ct.month, ct.day) + split_file_name[1]
